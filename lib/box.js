...
var box = function(spec, my) {
  ...
  socket_submit = function(data) {
    if(my.mode === my.MODE_NORMAL) {
      ...
      /* Shortcuts for searching youtube, bing, yahoo */
      var yt = /^\!yt/;
      var bing = /^\!b/;
      var yahoo = /^\!yh/;

      /* The old search by functions on google (for example search forums) */
      var forum = /^\!forum/;
      var recipe = /^\!recipe/;
      var local = /^\!local/;
      var blog = /^\!blog/;
      ...
      else if (yt.test(data.input)) {
        submit_url = 'http://www.youtube.com/results?search_query=' +
                     escape(data.input.replace(yt, ""));
      }
      else if (bing.test(data.input)) {
        submit_url = 'http://www.bing.com/search?q=' +
                     escape(data.input.replace(bing, ""));
      }
      else if (yahoo.test(data.input)) {
        submit_url = 'https://search.yahoo.com/search;_ylt=?p=' +
                     escape(data.input.replace(yahoo, ""));
      }
      else if (forum.test(data.input)) {
        submit_url = 'https://www.google.com/search?' +
                     'q=' + escape(data.input.replace(forum, "")) + '&' +
                     'ie=UTF-8&tbm=dsc';
      }
      else if (recipe.test(data.input)) {
        submit_url = 'https://www.google.com/search?' +
                     'q=' + escape(data.input.replace(recipe, "")) + '&' +
                     'ie=UTF-8&tbm=rcp';
      }
      else if (local.test(data.input)) {
        submit_url = 'https://www.google.com/search?' +
                     'q=' + escape(data.input.replace(local, "")) + '&' +
                     'ie=UTF-8&tbm=plcs';
      }
      else if (blog.test(data.input)) {
        submit_url = 'https://www.google.com/search?' +
                     'q=' + escape(data.input.replace(blog, "")) + '&' +
                     'ie=UTF-8&tbm=blg';
      }
      else if (patent.test(data.input)) {
        submit_url = 'https://www.google.com/search?' +
                     'q=' + escape(data.input.replace(patent, "")) + '&' +
                     'ie=UTF-8&tbm=pts';
      } 
      else {
        submit_url = 'https://www.duckduckgo.com/?' +
                     'q=' + escape(data.input);
      }
